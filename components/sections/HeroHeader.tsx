    ...
    <span aria-label={text} className="inline-block">
      {letters.map((char, i) => (
        <motion.span
          ...
          // Fix LCP + Marvin display: opacity-only (no y translation) so
          // the h1 occupies its space from the first paint, no layout shift.
...
function HeroBadge() {
  return (
    ...
    >
      ...
      <span className="text-white/60">Portfolio IT — Étudiant BTS SIO</span>
      <span className="text-accent-violet font-semibold">2025–2026</span>
    </motion.div>
  )
}
    ...
    >
      ...
      >
        ...
        <motion.p
          ...
          aria-label="Sous-titre : Portfolio IT"
        >
          — Portfolio IT —
        </motion.p>

        {/* ── Main Title with Anti-Gravity Effect ── */}
        <div className="mb-6" style={{ minHeight: '1.1em' }}>
          <h1
        ...
        <TiltCard className="glass rounded-card p-6 sm:p-8 mx-auto max-w-2xl mt-8 glow-violet">
          ...
          >
            <p className="text-xl sm:text-2xl font-light text-white/80 leading-relaxed">
              Étudiant en{' '}
              <span className="font-semibold gradient-text-violet" style={{ color: '#c4b5fd' }}>BTS SIO</span>
              {' '}passionné par les{' '}
